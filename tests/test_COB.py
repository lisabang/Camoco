...
def test_consistent_network_degree_dist(testCOB):
    ...
    merged_degree = old_degree.join(testCOB.degree, lsuffix="_old", rsuffix="_new")
    # Get rid of nans
    not_nan_mask = np.logical_not(
        np.isnan(merged_degree.Degree_old) & np.isnan(merged_degree.Degree_new)
    )
...
def test_consistent_GO_density(testCOB, testZmGO):
    ...
    # Get rid of nans
    not_nan_mask = np.logical_not(
        np.isnan(merged_densities.density_old) & np.isnan(merged_densities.density_new)
    )
    ...
    r2, pval = scipy.stats.pearsonr(
        merged_densities.density_new, merged_densities.density_old
    )
...
def test_coordination_between_expr_index_and_coex_index(testCOB):
    assert True
    return
    assert set(itertools.chain(*testCOB.coex.index.values)) == set(
        testCOB._expr.index.values
    )
...
def test_coex_score_concordance(testCOB):
    for a, b in itertools.combinations(
        ...
        assert (
            abs(testCOB.coexpression(a, b).score - testCOB._coex_concordance(a, b))
            < 0.001
        ) or (
            np.isnan(testCOB.coexpression(a, b).score)
...
def test_coex_to_expr_concordance(testCOB):
    ...
    expr_idxs = np.sort(
        np.unique(
            np.array([random.randint(0, expr_len - 1) for i in range(cf.test.num * 10)])
        )
    )
...
def test_num_neighbors_equals_degree(testCOB):
    ...
    assert len(testCOB.neighbors(random_gene)) == testCOB.global_degree(random_gene)
    assert len(
        testCOB.neighbors(random_gene, return_gene_set=True)
    ) == testCOB.global_degree(random_gene)
    assert len(
        testCOB.neighbors(random_gene, names_as_index=False)
    ) == testCOB.global_degree(random_gene)
...
def test_subnetwork_contains_only_input_genes(testCOB):
    ...
    assert set(itertools.chain(*subnet.index.values)) == set(
        [x.id for x in random_genes]
    )
