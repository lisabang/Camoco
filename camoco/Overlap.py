...
class Overlap(Camoco):
    ...
    def __init__(self, name):
        ...
        self.results = pd.read_sql(
            "SELECT * FROM overlap;", sqlite3.connect(self.db.filename)
        )
    ...
    def snp2gene(self, term, ont):
        if "effective" in self.args.snp2gene:
            # Map to effective
            return term.effective_loci(window_size=self.args.candidate_window_size)
    ...
    def SNP2Gene_breakdown(self, COB=None):
        ...
        snps.columns = pd.MultiIndex.from_product(
            [["GWAS SNPs"], ["-"], ["-"]], names=["Name", "WindowSize", "FlankLimit"]
        )
        snps.ix["Total"] = len(total.loci)
        # Calculate number of Candidate Loci
        loci = pd.pivot_table(
            results, index="Term", columns=["WindowSize"], values="TermCollapsedLoci"
        )
        for window_size in loci.columns:
            loci.ix["Total", window_size] = len(total.effective_loci(window_size))
        loci.columns = pd.MultiIndex.from_product(
            [["Collapsed Loci"], list(map(bp_to_kb, loci.columns)), ["-"]],
        ...
        for window_size in get_level(genes, "WindowSize"):
            for flank_limit in get_level(genes, "FlankLimit"):
                genes.ix["Total", (window_size, flank_limit)] = len(
                    ref.candidate_genes(
                        total.effective_loci(window_size=window_size),
    ...
    def high_priority_candidates(
        ...
        # df['fdr'] = fdr_cutoff
        df = (
            df.groupby(["COB", "Term", "Method", "gene"])
            .filter(lambda df: len(df) >= min_snp2gene_obs)
            .copy()
        )
        ...
        if not original_COB_only:
            ...
            both_same_net = (
                df.groupby(["COB", "Term", "gene"])
                .filter(lambda df: len(df.Method.unique()) == 2)
                .copy()
            )
            ...
            for term in gwas:
                for window in windows:
                    loci = term.strongest_loci(
                        attr="numIterations", window_size=window, lowest=False
                    )
                    for flank in flanks:
    ...
    def num_hpo(
        self, fdr_cutoff=0.3, min_snp2gene_obs=2, dropna=False, drop_empty_cols=True
    ):
        """
    ...
    def from_csv(cls, dir="./", sep="\t", name=None):
        ...
        results = pd.concat(
            [pd.read_table(x, sep=sep) for x in glob.glob(dir + "/*.overlap.tsv")]
        )
        if name is None:
    ...
    def plot_pval_heatmap(
        ...
        for i, cob in enumerate(cobs):
            for j, method in enumerate(methods):
                ...
                data = pd.pivot_table(
                    self.results.ix[
                        (self.results.COB == cob) & (self.results.Method == method)
                    ],
                ...
                if j == 0:
                    axis.set_yticklabels(axis.get_yticklabels(), size=7)
                    axis.set(yticks=np.arange(len(data.index)) + 0.5)
                else:
                    axis.set_yticks([])
